<?php
	...
	if($_SESSION == NULL) {
		...
		$assess_query = "SELECT `results`.`Assessment ID` FROM `objective results`
		RIGHT JOIN `results` ON `objective results`.`Assessment ID` = `results`.`Assessment ID` AND `results`.`NPI` = $_SESSION[npi]";
		$assess_res = $njhitec_db->query($assess_query);
		if($assess_res->num_rows >= 0) {
			$assess_num = $assess_res->fetch_array()["Assessment ID"];
		}
		else {
			$assess_num = 0;
		}
...
//	}
	...
	$category_query = "SELECT `Name`, `Failed Status`, `Achieved Status`, `Threshold`, `Total Objectives`
										 FROM `objective categories`";
	$category_results = $njhitec_db->query($category_query);
